...
const LeaseApartmentTable = (props) => {
    ...
    const rows = Array.apply(null, Array(props.apartment.apartmentLease.length)).map(
        (item, idx) => ({
            ...
            deposit: props.apartment.apartmentLease[idx].deposit.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","),
            month_fee: props.apartment.apartmentLease[idx].month_fee.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","),
